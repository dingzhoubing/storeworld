...
/**
 * changed some listener behavior of java source code
 * @author dingyuanxiong
 *
 */
public class GeneralCCombo extends Composite {
	...
	// record the column with the ccombo editor
	private static final int BRAND_COLUMN = 1;
	private static final int SUB_BRAND_COLUMN = 2;
	private static final int SIZE_COLUMN = 3;// no used anymore
	// record the name and area when in deliver common part
	...
	public GeneralCCombo(Composite parent, int style, int the_width,
			int the_col, String the_type) {
...
//									StockCellModifier.staticModify(item, "sub_brand", "");
						...
						} else if (col == SUB_BRAND_COLUMN) {//for sub brand
...
//								DeliverCellModifier.staticModify(item, "size", "");
		...
		text.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				boolean valid = false;
				if (col == NAME) {//if modify the text of name 
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (GeneralCCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == GeneralCCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
		...
		if ((style & SWT.SIMPLE) == 0) {
			...
			if (itemHeight != 0) {
				...
				visibleItemCount = Math.max(visibleItemCount, maxHeight
						/ itemHeight);
			}
		}
...
//		}
	...
	void listEvent(Event event) {
		...
		case SWT.FocusOut: {
			/*
			 * Behavior in Windows, GTK & Cocoa: When the arrow button is
			 * pressed with the popup list visible, the following events are
			 * received- popup control receives a deactivate event, list
			 * receives focus lost event, and then arrow button receives a
			 * selection event. If we hide the popup in the focus out event, the
			 * selection event will show it again. To prevent the popup from
			 * showing again, we will detect this case and let the selection
	...
	void textEvent(Event event) {
		...
		case SWT.MenuDetect: {
			...
			if (event.detail == SWT.MENU_KEYBOARD) {
				Point pt = getDisplay()
						.map(text, null, text.getCaretLocation());
				e.x = pt.x;
				e.y = pt.y;
			}
